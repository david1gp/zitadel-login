...
export async function mfaV2SmsOtpVerify(input: Input) {
  const op = "mfaV2SmsOtpVerify"
  ...
  const isEnrolled = input.state.mfaMethods.some(
    (method) => method === "AUTHENTICATION_METHOD_TYPE_OTP_SMS" || method === "sms_otp" || method === "otp_sms",
  )
  if (!isEnrolled) {
    return resultErrorCreate(op, "method_not_enrolled")
  }
  ...
  if (input.state.stage === "mfa_sms_otp_code" && input.state.challengeIssuedAt === undefined) {
  ...
  const methodIsCurrent =
    ...
    (currentOptions.mode === "check" && currentOptions.method.type === "sms_otp") ||
    (currentOptions.mode === "select" && currentOptions.methods.some((method) => method.type === "sms_otp"))
  if (!methodIsCurrent) return resultErrorCreate(op, "mfa_enrollment_not_allowed")

  const verified = await input.client.smsOtpSessionVerify(currentState.sessionId, currentState.sessionToken, input.code)
  if (!verified.success) {

...
export async function mfaV2SmsOtpChallenge(input: Input) {
  const op = "mfaV2SmsOtpChallenge"

  if (
    input.method !== undefined &&
    input.method !== "sms_otp" &&
    input.method !== "otp_sms" &&
    input.method !== "AUTHENTICATION_METHOD_TYPE_OTP_SMS"
  ) {
  ...
  const isEnrolled =
    (options.mode === "check" && options.method.type === "sms_otp") ||
    (options.mode === "select" && options.methods.some((method) => method.type === "sms_otp"))

  if (!isEnrolled) {
    return resultErrorCreate(op, "method_not_enrolled")
  }

  const challenged = await input.client.smsOtpSessionChallenge(currentState.sessionId, currentState.sessionToken)
  ...
  const state: Extract<FlowV2Cookie, { stage: "mfa" | "mfa_sms_otp_code" }> = {
    ...currentState,
    ...(input.state.stage === "mfa_sms_otp_code"
      ? {
          stage: "mfa_sms_otp_code" as const,
  ...
  const transition: FlowV2Transition = {
    ...
    screen:
      state.stage === "mfa_sms_otp_code"
        ? { name: "mfa_sms_otp_code", challengeIssued: true }
        : { name: "mfa", factors: state.mfaMethods },

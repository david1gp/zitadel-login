...
export async function mfaV2U2fVerifyApiRequest(
  apiOrigin: string,
  flowHandle: string,
  input: { credential: PasskeyCredentialAssertion; csrfToken: string; method?: "u2f" | "passkey" },
  fetchFn: typeof fetch = fetch,
): Promise<Result<FlowV2Transition>> {
  const op = "mfaV2U2fVerifyApiRequest"
  if (!flowHandle) return resultErrorCreate(op, flowV2ErrorMessageGet("flow_invalid"))

  const url = `${apiOrigin}/api/v2/mfa/u2f/verify?flow=${encodeURIComponent(flowHandle)}`

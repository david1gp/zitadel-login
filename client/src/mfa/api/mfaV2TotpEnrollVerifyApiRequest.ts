...
export async function mfaV2TotpEnrollVerifyApiRequest(
  apiOrigin: string,
  flowHandle: string,
  input: { code: string; csrfToken: string },
  fetchFn: typeof fetch = fetch,
): Promise<Result<FlowV2Transition>> {
  const op = "mfaV2TotpEnrollVerifyApiRequest"
  if (!flowHandle) return resultErrorCreate(op, flowV2ErrorMessageGet("flow_invalid"))
  if (!/^\d{6}$/.test(input.code)) return resultErrorCreate(op, flowV2ErrorMessageGet("code_invalid"))

  const url = `${apiOrigin}/api/v2/mfa/otp/enroll/verify?flow=${encodeURIComponent(flowHandle)}`

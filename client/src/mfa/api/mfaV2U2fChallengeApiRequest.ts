...
export async function mfaV2U2fChallengeApiRequest(
  apiOrigin: string,
  flowHandle: string,
  input: { method?: "u2f" | "passkey"; csrfToken: string },
  fetchFn: typeof fetch = fetch,
): Promise<Result<FlowV2Transition>> {
  const op = "mfaV2U2fChallengeApiRequest"
  if (!flowHandle) return resultErrorCreate(op, flowV2ErrorMessageGet("flow_invalid"))

  const url = `${apiOrigin}/api/v2/mfa/u2f/challenge?flow=${encodeURIComponent(flowHandle)}`

...
import {
  type MfaWebAuthnEnrollmentSetup,
  mfaWebAuthnEnrollmentSetupSchema,
} from "../model/mfaWebAuthnEnrollmentSetupSchema"

const responseSchema = v.strictObject({
  success: v.literal(true),
  data: mfaWebAuthnEnrollmentSetupSchema,
})
...
export async function mfaV2WebAuthnEnrollStartApiRequest(
  apiOrigin: string,
  flowHandle: string,
  input: { method: "u2f" | "passkey"; csrfToken: string },
  fetchFn: typeof fetch = fetch,
): Promise<Result<MfaWebAuthnEnrollmentSetup>> {
  const op = "mfaV2WebAuthnEnrollStartApiRequest"
  if (!flowHandle) return resultErrorCreate(op, flowV2ErrorMessageGet("flow_invalid"))

  const url = `${apiOrigin}/api/v2/mfa/${input.method}/enroll?flow=${encodeURIComponent(flowHandle)}`
  ...
  try {
    response = await fetchFn(url, {
      ...
      body: JSON.stringify({ method: input.method, csrfToken: input.csrfToken }),
    })
  } catch (error) {

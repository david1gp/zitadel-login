...
export async function mfaV2SmsOtpVerifyApiRequest(
  apiOrigin: string,
  flowHandle: string,
  input: { code: string; csrfToken: string },
  fetchFn: typeof fetch = fetch,
): Promise<Result<FlowV2Transition>> {
  const op = "mfaV2SmsOtpVerifyApiRequest"
  if (!flowHandle) return resultErrorCreate(op, flowV2ErrorMessageGet("flow_invalid"))

  const url = `${apiOrigin}/api/v2/mfa/sms-otp/verify?flow=${encodeURIComponent(flowHandle)}`
  ...
  try {
    response = await fetchFn(url, {
      ...
      body: JSON.stringify({ code: input.code, method: "sms_otp", csrfToken: input.csrfToken }),
    })
  } catch (error) {

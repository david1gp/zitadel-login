...
export async function mfaV2SkipApiRequest(
  apiOrigin: string,
  flowHandle: string,
  input: { csrfToken: string },
  fetchFn: typeof fetch = fetch,
): Promise<Result<FlowV2Transition>> {
  const op = "mfaV2SkipApiRequest"
  if (!flowHandle) return resultErrorCreate(op, flowV2ErrorMessageGet("flow_invalid"))

  const url = `${apiOrigin}/api/v2/mfa/skip?flow=${encodeURIComponent(flowHandle)}`
  ...
  try {
    response = await fetchFn(url, {
      ...
      body: JSON.stringify({ csrfToken: input.csrfToken }),
    })
  } catch (error) {

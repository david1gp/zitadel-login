...
export async function mfaV2EmailOtpEnrollApiRequest(
  apiOrigin: string,
  flowHandle: string,
  input: { csrfToken: string },
  fetchFn: typeof fetch = fetch,
): Promise<Result<FlowV2Transition>> {
  const op = "mfaV2EmailOtpEnrollApiRequest"
  if (!flowHandle) return resultErrorCreate(op, flowV2ErrorMessageGet("flow_invalid"))

  const url = `${apiOrigin}/api/v2/mfa/email-otp/enroll?flow=${encodeURIComponent(flowHandle)}`

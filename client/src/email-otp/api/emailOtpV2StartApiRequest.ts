...
export async function emailOtpV2StartApiRequest(
  apiOrigin: string,
  flowHandle: string,
  input: { email: string; csrfToken: string },
): Promise<Result<FlowV2Transition>> {
  const op = "emailOtpV2StartApiRequest"
  const url = new URL("/api/v2/email-otp/start", apiOrigin || window.location.origin)

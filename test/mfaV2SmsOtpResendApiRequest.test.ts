...
import { mfaV2SmsOtpResendApiRequest } from "../client/src/mfa/api/mfaV2SmsOtpResendApiRequest"

const apiOrigin = "https://worker.example"
const flowHandle = "flow-handle-123"
const csrfToken = "B".repeat(43)

describe("mfaV2SmsOtpResendApiRequest", () => {
  test("sends POST request to /api/v2/mfa/sms-otp/resend and returns render transition on success (202 status)", async () => {
    let capturedUrl = ""
    ...
    expect(capturedUrl).toBe(`${apiOrigin}/api/v2/mfa/sms-otp/resend?flow=${flowHandle}`)
  ...
  test("returns error if flow handle is missing", async () => {
    const res = await mfaV2SmsOtpResendApiRequest(apiOrigin, "", { csrfToken })

...
function nativeCreate(options: NativeOptions = {}) {
  ...
  const fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    if (url === `${identityOrigin}/v2/sessions/session-1` && method === "PATCH") {
      if (options.emailOtpStatus) {
        return Response.json({ error: "email otp check failed" }, { status: options.emailOtpStatus })
      }
      ...
      return Response.json({
        session: {
          ...
          factors: options.factors ?? {
            ...
            otpEmail: { verifiedAt: new Date(now * 1000).toISOString() },
          },
        },
      })
    }
...
describe("Worker V2 MFA Email OTP Verification Flow", () => {
  test("completes valid email OTP verification and transitions to verified stage via /api/v2/mfa/email-otp/verify (supporting 6-20 digit formats including deployed 8 digits)", async () => {
    ...
    for (const code of codes) {
      ...
      const response = await app.request(
        `https://login.example/api/v2/mfa/email-otp/verify?flow=${flowHandle}`,
        {
          method: "POST",
          headers: {
            origin,
  ...
  test("works identically via /api/v2/mfa/otp/verify endpoint when method is email_otp", async () => {
    const native = nativeCreate()
    const app = workerAppCreate({
      fetch: native.fetch,
      now: () => now,
  ...
  test("rejects invalid or expired email OTP codes with 401 code_invalid", async () => {
  ...
  test("rejects verification when email OTP is already-used as primary factor or not enrolled", async () => {
    const native = nativeCreate()
    const app = workerAppCreate({
      fetch: native.fetch,
      now: () => now,
    ...
    const cookie = await flowCookieCreate({
      mfaMethods: ["AUTHENTICATION_METHOD_TYPE_TOTP"], // email_otp not enrolled
    })
    const response = await app.request(
      `https://login.example/api/v2/mfa/email-otp/verify?flow=${flowHandle}`,
      {
  ...
  test("verifies response contains no sensitive secrets or contact info", async () => {
    const native = nativeCreate()
    ...
    expect(rawText).not.toContain("updated-secret-mfa-token")
    expect(rawText).not.toContain("secret-user@example.com")

    for (const log of logs) {
      const logStr = JSON.stringify(log)
      expect(logStr).not.toContain("12345678")
      expect(logStr).not.toContain("updated-secret-mfa-token")

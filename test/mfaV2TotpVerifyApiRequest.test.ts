...
import { mfaV2TotpVerifyApiRequest } from "../client/src/mfa/api/mfaV2TotpVerifyApiRequest"

const apiOrigin = "https://worker.example"
const flowHandle = "flow-handle-123"
const csrfToken = "B".repeat(43)

describe("mfaV2TotpVerifyApiRequest", () => {
  test("sends POST request to /api/v2/mfa/totp/verify and returns complete transition on success", async () => {
    let capturedUrl = ""
    ...
    expect(capturedUrl).toBe(`${apiOrigin}/api/v2/mfa/totp/verify?flow=${flowHandle}`)
    expect(capturedBody).toEqual({ code: "123456", method: "totp", csrfToken })
    expect(res).toEqual({
      success: true,
      data: {
        kind: "complete",
  ...
  test("returns error if flow handle is missing", async () => {
    const res = await mfaV2TotpVerifyApiRequest(apiOrigin, "", { code: "123456", csrfToken })
    expect(res.success).toBe(false)
    if (res.success) return
